...
async def patch_user_email(
    new_email: EmailStr, user: Annotated[UserModel, Depends(depends.get_current_user)]
) -> dict[str, str]:
...
async def patch_user_password(
    ...
    if result:
        return await UserCrud.patch_user_password(
            username=user.username, password=hash_new_password
        )
    raise HTTPException(
